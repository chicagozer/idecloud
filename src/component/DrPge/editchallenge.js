...
class EditChallange extends Component {
  ...
  save = e => {
    ...
    if (
      ...
      axios
        .put("http://127.0.0.1:8000/challenge/" + this.props.editid, {
        ...
        .then(res => {
          this.state.id.map(id =>
            axios
              .get("http://127.0.0.1:8000/Texttestcase/" + id, {})
              .then(res => {
                axios
                  .put("http://127.0.0.1:8000/Texttestcase/" + id, {
                    challenge: this.props.editid,
                    Input: this.state.input.get(parseInt(res.data.number, 10)),
                    output: this.state.output.get(
                      parseInt(res.data.number, 10)
                    ),
                    visable: this.state.visible
                      .get(parseInt(res.data.number, 10))
                      .toString(),
                    weight: this.state.percent.get(
                      parseInt(res.data.number, 10)
                    ),
                    number: parseInt(res.data.number, 10)
                  })
                  .then(res => {})
                  .catch(err => {
                    console.log(err.message);
      ...
      if (this.state.testcase.length > this.state.id.length) {
            ...
            <Col span={24}>
              {this.state.testcase.map(item => (
                ...
                >
                  ...
                  <InputLabel style={{ color: "#305656" }}> Weight </InputLabel>
                  <Input
                    style={{
                      width: 70,
                      height: 35,
